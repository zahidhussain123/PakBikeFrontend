...
class PostAdDetail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      users: [],
  ...
  // update method
  update(_id) {
    fetch("http://localhost:3000/Ad/adPut/" + _id, {
      method: "PUT",
      // headers: { Authorization: `Bearer ${token}` },
    }).then((results) => {
      results.json().then((resp) => {
    ...
    return (
      <div>
        <div className="heading_records_postad_details">
          <div className="heading_records_setting">All Bike Ad Details</div>
        </div>
        <div className="All_records_postad_details">
          ...
          <div className="price_ad_detail_post">Price</div>
