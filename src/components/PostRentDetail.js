...
class PostRentDetail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      rentList: [],
  ...
  // update method
  update(_id) {
    fetch("http://localhost:3000/Rent/rentPut/" + _id, {
      method: "PUT",
      // headers: { Authorization: `Bearer ${token}` },
    }).then((results) => {
      results.json().then((resp) => {
    ...
    return (
      <div>
        <div className="heading_records_postad_details">
          <div className="heading_records_setting">All Rent Bikes Details</div>
        </div>
